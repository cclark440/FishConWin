...
namespace FishConWn
{
    class FishConVars
    {
        ...
        public static String SOQuery = "SELECT so.id, soi.productNum AS Part, soi.qtyOrdered AS QTY, cuset.info AS Label, so.ShipToName FROM so JOIN soitem AS soi ON so.id = soi.soid LEFT JOIN customset AS cuset ON cuset.recordid = soi.productid AND cuset.customfieldid = 94 where so.num='";
